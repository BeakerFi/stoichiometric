...
impl LenderState {
    ...
    pub fn from(collateral_amount: Decimal,
                loan_to_value: Decimal,
                interest_rate: Decimal,
                liquidation_threshold: Decimal,
                liquidation_incentive: Decimal) -> Self
    {
        Self {
            collateral_amount,
            loan_to_value,
    ...
    fn assert_lenders_state(lenders_1: &HashMap<String, LenderState>, lenders_2: &HashMap<String, LenderState>) {
...
impl IssuerState {
    ...
    pub fn update(&mut self) {
        ...
        let output = run_command(Command::new("resim").arg("show").arg(&self.component_address));
        ...
        lazy_static!{
            static ref LENDER_ADDRESS_RE: Regex = Regex::new(r#"ResourceAddress\("(\w*)"\)"#).unwrap();
        }
        ...
        for lender_address in LENDER_ADDRESS_RE.captures_iter(lenders_map)
        {
            ...
            let new_output = run_command(Command::new("resim").arg("call-method").arg(&self.component_address).arg("get_lender_state").arg(&token));
    ...
    pub fn assert_state_is(&self, reserves: &HashMap<String, Decimal>, lenders: &HashMap<String, LenderState>, loan_id: u64, flash_mint_id: u64) {
