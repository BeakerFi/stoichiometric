...
use sqrt::method::Arg::{ComponentAddressArg, DecimalArg, I64, NonFungibleBucketArg, ResourceAddressArg, StringArg, VecArg};
...
pub enum DaoMethods {
    ...
    MakeAddNewCollateralToken(String, Decimal, Decimal, Decimal, Decimal, Decimal, Decimal, Decimal, String),
...
impl Method for DaoMethods {
    ...
    fn args(&self) -> Option<Vec<Arg>> {
        match self
        {
            DaoMethods::Unlock(voter_card_id) => {
                method_args!(
                    NonFungibleBucketArg(VOTER_CARD_NAME.to_string(), vec![voter_card_id.clone()])
                )
            }
            DaoMethods::MakeChangeVotePeriodProposal(vote_period) =>
                {
                    method_args!(
                        enum_arg!(0, I64(vote_period.clone()))
                    )
                }
            DaoMethods::MakeMinimumVoteThresholdProposal(vot_threshold) => {
                    method_args!(
                        enum_arg!(
                            1,
                            DecimalArg(vot_threshold.clone())
                        )
                    )
                }
            DaoMethods::MakeGrantIssuingRightProposal => {
            ...
            DaoMethods::MakeAllowClaimProposal(resources) =>
                {
                    ...
                    for (resource, amount) in resources {
                        vec_arg.push(tuple_arg!(StringArg(resource.clone()), DecimalArg(amount.clone())))
                    }
                    method_args!(
                        enum_arg!(3, VecArg(vec_arg))
                    )
                }
            DaoMethods::MakeAddNewCollateralToken(token, loan_to_value, interest_rate, liquidation_threshold, liquidation_penalty, initial_rate, minimum_rate, maximum_rate, oracle) =>
                {
                    method_args!(
                        enum_arg!(4, ResourceAddressArg(token.clone()), DecimalArg(loan_to_value.clone()), DecimalArg(interest_rate.clone()), DecimalArg(liquidation_threshold.clone()), DecimalArg(liquidation_penalty.clone()), DecimalArg(initial_rate.clone()), DecimalArg(minimum_rate.clone()), DecimalArg(maximum_rate.clone()), ComponentAddressArg(oracle.clone()))
                    )
                }
            DaoMethods::MakeChangeLenderParameters(lender, loan_to_value, interest_rate, liquidation_threshold, liquidation_penalty) => {
                method_args!(
                    enum_arg!(5, ResourceAddressArg(lender.clone()), DecimalArg(loan_to_value.clone()), DecimalArg(interest_rate.clone()), DecimalArg(liquidation_threshold.clone()), DecimalArg(liquidation_penalty.clone()))
                )
            }
            DaoMethods::MakeChangeLenderOracle(lender, oracle) =>
                {
                    method_args!(
                        enum_arg!(6, ResourceAddressArg(lender.clone()), ComponentAddressArg(oracle.clone()))
                    )
                }
            DaoMethods::MakeAddTokensToIssuerReserves(resources) =>
                {
                    ...
                    method_args!(
                        enum_arg!(7, VecArg(vec_arg))
                    )
                }
            DaoMethods::ExecuteProposal(proposal_receipt_id) =>
                {
                    method_args!(
                        NonFungibleBucketArg(PROPOSAL_RECEIPT.to_string(), vec![proposal_receipt_id.clone()])
                    )
                }
    ...
    fn custom_manifest_name(&self) -> Option<&str> {
        match self {
            ...
            DaoMethods::MakeMinimumVoteThresholdProposal(_) => { Some("make_minimum_vote_threshold_proposal") }
            ...
            DaoMethods::MakeAddNewCollateralToken(_, _, _, _, _, _, _, _, _) => { Some("make_add_new_collateral_proposal") }
            DaoMethods::MakeChangeLenderParameters(_, _, _, _, _) => { Some("make_change_lender_parameters_proposal") }
            DaoMethods::MakeChangeLenderOracle(_, _) => { Some("make_change_lender_oracle_proposal") }
            DaoMethods::MakeAddTokensToIssuerReserves(_) => { Some("make_add_tokens_to_issuer_reserves_proposal") }
